...
def pub_news():
    if request.method == 'GET':
        ...
        if form.validate():
            ...
            news = News(title=title,type=int(select),content=content,author_id=author_id)
            db.session.add(news)
            db.session.commit()
...
@app.route('/news/<id>')
def news_detail(id):
    _news = News.query.get(id)
    comments = Comment.query.filter_by(news_id=id).order_by(Comment.pub_time.desc()).all()
    return render_template('news_detail.html',news=_news,comments=comments)
...
def add_comment():
    ...
    author_id = current_user.id
    content = request.form.get('ckeditor')
    comment = Comment(content=content,author_id=author_id,news_id=news_id)
...
@app.route('/dcollect/',methods=['POST'])
def dcollect():
    user_id = request.form.get('user_id')
    news_id = request.form.get('news_id')
    news = News.query.get(news_id)
